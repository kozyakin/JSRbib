...
# To make this file compatible with tex4ht, modifications were made on 2020.07.15
...
Usage: $0 [-c] [-d] [-h] [-i] [-k] [-p] [-r] [filename ...]

-c generate .ain file with citation references (added by V.Kozyakin)
...
my %Aut2Cite;		# comma separated list of BibTeX entry labels an author occurs in
...
if(exists $opt{'i'}){
	...
	if($Aut2Lab{$author}){
        ...
        if(exists $opt{'c'}){
            $rep=~ tr/, /,\~/;
            print AIFILE "\\item[\\mbox\{$rep\}]";
            print AIFILE "\~\\cite\{@{$Aut2Cite{$author}}\}\n";
	}else{
